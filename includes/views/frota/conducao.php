				...
				<tbody>
					...
					else
					{
						while ($conducao = $result->fetch_assoc()) 
						{
							...
							echo '<td class="text-center" nowrap><a href="'. (!empty($locFinal) ? 'https://www.google.com/maps/search/'.$locFinal['latitude'].','.$locFinal['longitude'].'/' : '#') .'" target="_blank">'.date('H:i', strtotime($conducao['data_final'])).' (<small>'.(!empty($locFinal) ? GEO::ObterEnderecoPorCoords($locFinal['latitude'], $locFinal['longitude']) : 'Indefinido').'</small>)</a></td>';
