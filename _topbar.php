...
$mensagens = $_SESSION['user']->ObterMensagens(4);
$msgsPorLer = 0;
$alertas = $_SESSION['user']->ObterAlertas(4);
$alertasPorLer = 0;

// Contar mensagens por ler
foreach ($mensagens as $msg)
	if(!$msg['lida'])
		$msgsPorLer++;

// Contar os alertas por ler
	...
	<ul class="navbar-nav ml-auto">
		...
		<li class="nav-item dropdown no-arrow mx-1">
			<a class="nav-link dropdown-toggle" href="#" id="alertsDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
				...
				<?=  $alertasPorLer ? '<span class="badge badge-danger badge-counter">'.$alertasPorLer.'</span>' : NULL ?>
			</a>
			<!-- Alertas - Dropdown -->
			<div class="dropdown-list dropdown-menu dropdown-menu-right shadow animated--grow-in" aria-labelledby="alertsDropdown">
				...
				foreach ($alertas as $alerta)
				{
					...
					echo '
					<a class="dropdown-item d-flex align-items-center" href="index.php?'.$alerta['link'].'">
						<div class="mr-3">
							<div class="icon-circle bg-primary"><i class="fas fa-file-alt text-white"></i></div>
						</div>
						<div>
							<div class="small text-gray-600">'.timeago($alerta['data']).'</div>
							<span'.(!$_lido ? ' class="font-weight-bold"' : NULL).'>'.$alerta['descricao'].'</span>
						</div>
					</a>
					';
				}
		...
		<li class="nav-item dropdown no-arrow mx-1">
			<a class="nav-link dropdown-toggle" href="#" id="messagesDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
				...
				<?= $msgsPorLer ? '<span class="badge badge-danger badge-counter">'.$msgsPorLer.'</span>' : NULL ?>
			</a>
			<!-- Mensagens - Dropdown -->
